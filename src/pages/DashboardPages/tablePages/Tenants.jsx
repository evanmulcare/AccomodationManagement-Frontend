...
const Tenants = () => {
  ...
  useEffect(() => {
    Axios.get('https://accomodationmanagementbackend.onrender.com/tenants')
      .then(res => {
        console.log(res)
        setData(res.data)
      })
  ...
  const tenantDelete = (id, e) => {
    e.preventDefault();
    Axios.delete(`https://accomodationmanagementbackend.onrender.com/tenants/${id}`)
  ...
  const exportToExcel = () => {
    ...
    utils.book_append_sheet(workbook, worksheet, 'Tenants');
    writeFile(workbook, 'tenants.xlsx');
  };
      ...
      <tr className="bg-white shadow-md">
        ...
        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          <div className="flex items-center justify-center space-x-2">
            ...
            <button
              onClick={(e) => tenantDelete(data._id, e)}
      ...
      <div className='bg-white rounded-lg shadow-md p-4 space-y-4'>
        ...
        <div className='flex items-center justify-between'>
          <span className='p-2 text-md font-medium tracking-wider bg-green-200 text-green-800 hover:bg-green-300 rounded-lg bg-opacity-50 flex justify-around'>
            <Link className='hover:underline' to={`/tenant/${data._id}`}>
              <GrView />
            </Link>
            <span className='text-gray-500'>
              <RxDividerVertical />
            </span>
            <button onClick={(e) => tenantDelete(data._id, e)} className='hover:underline'>
              <MdPersonRemoveAlt1 />
            </button>
          </span>
        </div>
  ...
  return (
    <div className='m-2 md:m-10 p-2 md:p-10 bg-white rounded-3xl '>
      ...
      <div className='flex justify-between items-center mb-6'>
        ...
        <div>
        <Link to="/new-tenant">
          <button className='bg-lime-700 hover:bg-lime-600 text-white font-sm py-2 px-4 mx-2 rounded-md'>Add Tenant</button>
        </Link>
        <button 
        ...
        <table className='w-full'>
          <thead className='bg-gray-50 border-b-2 border-gray-200'>
            <tr>
              ...
              <th className='w-1/5 p-3 text-left text-xs font-semibold tracking-wider text-gray-500 uppercase'>CID</th>
